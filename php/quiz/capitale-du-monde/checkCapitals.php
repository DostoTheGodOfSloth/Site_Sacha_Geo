                    ...
                    <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3 boxQuiz-1">
                        ...
                        <div class="flexForm">
                            <?php 
                                ...
                                if(isset($_POST['btnCheck']) && isset($_POST['capitals']) && isset($_SESSION['pays']))
                                {
                                    $score = 0;
                                    /*Prend aussi l'index pour récupérer les capitales de l'autre page*/
                                    foreach($_POST['capitals'] as $index => $namePays)
                                    {
                                        $pay = $_SESSION['pays'][$index];
                                        /*met en miniscule l'insertion et la db pour la correlation */
                                        if(removeAccents(trim($namePays)) == removeAccents(trim($pay['payCapitale'])))
                                        {
                                            $score++;
                                            echo "
                                                <div class=' cardQuiz'>
                                                    <p class='textCard'>" .$pay['payNom'] . "</p>
                                                    <img src='../../../img/drap/" .$pay['idPays'] .".png' class='imgCapital-quiz' alt=s'Drapeau d'un pays'>
                                                    <div class='form__group field'>
                                                        <input type='input' class='form__field right' value='".$pay['payCapitale']."' disabled/>
                                        ...
                                        else
                                        {
                                            echo "
                                                <div class=' cardQuiz'>
                                                    ...
                                                    <div class='form__group field'>
                                                        <input type='input' class='form__field wrong' value='".$pay['payCapitale']."' disabled/>
                                                        <label for='name' class='form__label-1'>Nom</label>
                                                    </div>
                                                </div>
                                            ";
                                    ...
                                    echo"<div class='score'>";
                                        ...
                                        echo"<div class='score-2'>";
                                            echo"<a href='capitals.php' class='buttonUp-1'>Recommencez</a>";
                                        echo"</div>";
                                    echo"</div>";

                                    if(isset($_SESSION['idUser']))
                                    {
                                        $database->addScore($_SESSION['idUser'], '3', strval($score), $timer);
                                    }
                                }
                            ?>
                        </div>
